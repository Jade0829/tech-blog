...
## 인사말
  ...
  작년 국가정보자원관리원에서 발생한 데이터 센터 화재와 같은 재난 상황에서도 시스템이 정상적으로 동작하려면 데이터 손실 없이 효과적인 백업 및 복구 전략이 필수적입니다. 재해 복구 계획에서 RPO[^1]는 가장 중요한 지표 중 하나로, RPO가 짧을수록 재난 발생 시 더 많은 데이터를 보호할 수 있어 비즈니스 연속성을 보장할 수 있습니다. 특히 Lustre와 같은 분산 파일시스템에서는 전체 데이터를 주기적으로 동기화하는 전통적인 방식보다, 변경된 부분만 효율적으로 추적하고 복제할 수 있는 방법이 중요합니다. 이를 통해 RPO를 최소화하고 실시간에 가까운 데이터 보호를 실현할 수 있습니다.

  이번 블로그에서는 Lustre 파일시스템에서 파일 생성, 수정, 삭제와 같은 모든 이벤트를 기록하는 changelog[^2]와 이를 활용한 글루시스의 DR(Disaster Recovery) 솔루션을 소개합니다.

  이번 내용을 이해하는 데 도움이 되도록, 이전 블로그 [Lustre_FID_1](https://tech.gluesys.com/blog/2025/03/13/Lustre_FID_1.html)과 [Lustre_FID_2](https://tech.gluesys.com/blog/2025/09/01/Lustre_FID_2.html)를 참고하시기 바랍니다. 이전 블로그에서는 Lustre 파일시스템에서 파일 생성 시 내부 동작 방식과 Lustre가 파일을 고유한 FID(File Identifier)로 관리하는 방식을 다루었습니다. 이러한 기초 지식은 이번 블로그에서 설명하는 Changelog와 DR 솔루션의 동작 원리를 이해하는 데 중요한 배경이 됩니다.

&nbsp;

## 전통적인 rsync 방식의 한계와 파일 히스토리 기반 DR

  DR을 구현하기 위해 전통적으로 사용되는 방식은 rsync[^3]를 활용한 전체 스캔 기반 동기화입니다. 이 방식은 SSH 명령어 또는 SSH 데몬을 통해 실행할 수 있으며, 전체 파일시스템을 주기적으로 스캔하여 변경된 파일을 찾아 동기화합니다.

  그러나 rsync 기반 동기화 방식은 파일시스템을 전체 스캔한 후 동기화를 수행하기 때문에 파일시스템 크기에 비례하여 스캔 시간이 증가합니다. 대규모 파일시스템(Lustre)에서는 동기화에 수십 시간이 소요될 수 있으며, 동기화가 스캔 주기에 의존하기 때문에 RPO를 최소화하기 어렵습니다. 이러한 한계점들로 인해 대규모 파일시스템 환경에서 실시간에 가까운 DR을 구현하기 위해 전통적인 rsync 방식을 사용한다면 많은 비용이 소모됩니다

  이를 해결하기 위해 파일 히스토리를 기반으로 한 DR 방식을 고려할 수 있습니다. 파일 히스토리를 기반으로 수정된 파일 목록만을 이용하여 동기화한다면 기존 rsync의 전체 스캔에 소요되는 시간을 크게 줄일 수 있습니다. 또한 모니터링을 통해 지속적으로 수정된 파일 목록을 받을 수 있다면 적은 비용으로 실시간에 가까운 DR을 구현할 수 있습니다.

  파일 히스토리 기반 DR은 다음과 같은 이점을 제공하여 `데이터 센터 간 백업`, `재해 복구`, `지역별 복제`, `개발/운영 환경 동기화` 같은 시나리오에서 효과적으로 활용할 수 있습니다.
...
#### 실시간성
- **이벤트 발생 즉시 처리**: 파일 변경 이벤트가 발생하는 즉시 동기화를 시작하여 RPO를 최소화
...
## 글루시스 DR 솔루션
  ...
  앞서 설명한 Lustre의 Changelog를 활용한 글루시스의 DR 솔루션은 실시간으로 파일시스템 변경 사항을 원격 사이트에 동기화하는 시스템입니다. 전통적인 전체 스캔 기반 동기화 방식과 달리, 변경된 파일만 효율적으로 추적하고 복제하여 네트워크 대역폭과 처리 시간을 크게 절감할 수 있습니다.
...
- **이벤트 큐**: Redis Stream[^4]을 사용하여 이벤트를 비동기적으로 전달.

- **원격 통신**: gRPC[^5]를 사용하여 타겟 서버와 통신. gRPC는 HTTP/2 기반의 고성능 RPC 프레임워크로, 타입 안전성과 효율적인 직렬화를 제공합니다. 파일 메타데이터 작업에 최적화되어 있습니다.
...
**rsync를 통한 동기화 (CREATE/MODIFY 이벤트)**
...
**gRPC를 통한 동기화 (RENAME/DELETE 이벤트)**
...
### 테스트 결과
  ...
  아래 표는 시장에서 요구하는 재해 복구 솔루션에 대한 등급표입니다. 해당 표를 기준으로 글루시스 DR 솔루션의 RPO와 RTO[^6]를 평가하여 어떤 등급에 해당하는지 확인해보겠습니다.
...
|-----|--------|---------|--------|---------|-------|
| 큰큰 파일 | 55M | 10,000 | 412 MB/s | 39s | 19m40s |
| 작은 파일 | 314K | 10,000 | 24.2 MB/s | 0.32s | 2m42s |

  파일 동기화는 서비스 복구 과정에서 가장 먼저 안정적으로 동작해야 하는 핵심 단계입니다. 글루시스 DR 솔루션의 테스트 결과를 살펴보면, 데이터는 실시간으로 동기화되며 상대적으로 작은 파일의 경우 약 3분 이내, 파일 사이즈가 큰 업무 환경에서 생성되는 파일의 경우에도 약 20분 이내에 동기화가 완료됩니다. 이는 장애 발생 시 데이터 손실이 거의 발생하지 않는 수준으로, 실질적으로 RPO가 0에 가깝게 유지됨을 의미합니다.
  &nbsp;
  전체 서비스 복구 시간은 기업의 내부 시스템 구성과 운영 환경에 따라 차이가 있을 수 있으나, 실제 운영 시나리오를 기준으로 보면 수 시간에서 최대 하루 이내에 서비스 복구가 가능한 수준으로 볼 수 있습니다. 이러한 동작 특성을 앞서 정리한 등급별 요구 수치와 비교해보면, 글루시스 DR 솔루션은 Platinum 또는 Gold 등급 수준의 요구사항을 만족하는 범위에 해당한다고 볼 수 있습니다.
...
## 마치며
  ...
  이번 블로그에서는 Lustre 파일시스템의 DR(Disaster Recovery) 솔루션을 주제로, 전통적인 rsync 방식의 한계와 이를 극복하기 위한 Lustre Changelog 기반 DR 솔루션에 대해 알아보았습니다.
...
[^1]: RPO(Recovery Point Objective) : 재해 복구에서 허용 가능한 최대 데이터 손실 시간 
&nbsp;
...
&nbsp;
[^6]: RTO(Recovery Time Objective) : 업무의 복구 목표 시간
&nbsp;
