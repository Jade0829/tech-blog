...
- **모니터링**: 실시간성이 중요한 모니터링 작업을 위해, 해당 컴포넌트는 C 언어로 구현되어 Lustre API를 직접 호출하여 이벤트를 수집합니다. 이는 `lfs changelog`와 같은 사용자 레벨 도구를 통해 간접적으로 Changelog를 조회하는 방식보다, 호출-응답 지연을 최소화하고 불필요한 프로세스 오버헤드를 줄일 수 있어 성능과 리소스 효율 측면에서 유리하기 때문입니다.
...
- **설정 관리**: JSON 형식은 가독성이 좋고 다양한 프로그래밍 언어에서 쉽게 파싱할 수 있어 JSON 파일로 관리합니다.
...
1. **모니터링 컴포넌트**: Lustre Changelog를 실시간으로 모니터링하고 이벤트를 수집합니다.
2. **소스 서버 컴포넌트**: 소스 서버에서 실행되며, 수집된 이벤트를 처리하고 동기화 작업을 수행합니다.
3. **타겟 서버 컴포넌트**: 타겟 서버에서 실행되며, 원격에서 전송된 파일 작업 요청을 처리합니다.
...
**재시도 스트림을 통한 누락 방지**
  ...
  rsync 재동기화 스트림과 gRPC 재동기화 스트림에 저장됩니다.
